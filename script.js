...
//console.log(context);
/*Canvas จะต้องใช้คำ x (width) เเละ ค่า y (height) เพื่อกำหนดตำเเหน่งใน Canvas
.moveTo กำหนดจุดเริ่มต้นของเส้น
.lineTo กำหนดเส้นที่ลากจากจุดเริ่มต้นไปที่จุดสุดท้าย
.lineWidth กำหนดความหนาของเส้น
.stokeStyle กำหนดสี
.stroke() function กำหนดให้ทุกอยากเปลี่ยนสี .strokeStyle
.beginPath() คำสั่งที่ใช้ในการขึ้น Path ใหม่
.clorePath() คำสั่งจบการทำงานของ Path
.fillstyle กำหนดสีเเละเเรงเงา
.fill() ระบายสี
.fillRect(x,y,width,height) function สร้างสามเหลี่ยม
.strokeRect(x,y,width,height) function สามเหลี่ยมเริ่มต้นไม่มีสี
.fillText(message,x,y) function สร้างข้อความตามตำเเหน่ง x ,y
.strokeText(message,x,y) สร้างข้อความตามตำเเหน่ง x ,y เริ่มต้นไม่มีสี
.font = [normall][italic][normal  bold][ขนาด][ชื่อ Font]
.drawImage(รูปภาพที่ต้องการ ,x,y,width,height) function เเสดงรูปภาพนั้นๆ ตามตำเเหน่ง
...
// ฟังชันตั้งค่าเสียง
let jumpSound = new Audio("กระโดด.mp3");
let deathSound = new Audio("ตาย.mp3");
let collectSound = new Audio("เก็บของ.mp3"); 
...
window.onload = function() {
    ...
    backgroundImg.src = "mare88.jpg";

    // โหลดภาพตัวละคร
    playerImg = new Image();
    playerImg.src = "mare.png";

    // สร้างอุปสรรค 
    boxImg = new Image();
    boxImg.src = "mare4.png";

    // โหลดภาพไอเทม
    itemImg = new Image();
    itemImg.src = "mare3.png";
...
function update() {
    ...
    for(let i = 0 ; i < boxesArray.length ; i++ ) {
        ...
        if(onCollision(player,box)) {
            ...
            if (lives > 0) {
                context.fillText("Ouch! You hit a box.", boardWidth / 2, boardHeight / 2 - 20);
                ...
                context.font = "bold 25px Arial";
                context.fillText("No lives left. You cannot play anymore.", boardWidth / 2, boardHeight / 2 + 20);
    ...
    if (time >= 60) {
        ...
        context.font = "bold 40px Arial";
        context.textAlign = "center";
        context.fillText("Time's Up! You Survived!", boardWidth / 2, boardHeight / 2);
    ...
    if((e.code == "Space" || e.key == " " || e.code == "ArrowUp") && player.y >= playerY) {
        ...
        try {
            ...
            jumpSound.play().catch(error => {
                console.log("เว็บบราวเซอร์บล็อกการเล่นเสียงอัตโนมัติ ให้คลิกที่หน้าจอเกม 1 ครั้งก่อนเล่นครับ");
            }); 
        } catch (err) {
            console.log(err);
        }
...
function restartGame() {
    ...
    if (lives <= 0) {
        alert("ทำไมเดินไม่ดู ชนเลยเนี่ย");
        return; 
    }
